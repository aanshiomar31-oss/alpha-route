...
from alpharoute.brokers.noren import *
...
def broker():
    ...
    with patch("alpharoute.brokers.noren.NorenApi") as mock_broker:
...
@pytest.fixture
def mod():
    YAML_PATH = PurePath(__file__).parent.parent.parent / "alpharoute"
...
def test_authenticate():
    ...
    with patch("alpharoute.brokers.noren.BaseNoren") as mock_broker:
        broker = Noren(
...
def test_orders_invalid_broker_timestamp(broker):
    ...
    order1 = Order(**fetched[0])
    assert order1.broker_timestamp is None
