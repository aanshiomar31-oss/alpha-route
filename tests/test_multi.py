...
def test_multi_user_order_place_broker(users_simple):
    multi = MultiUser(users=users_simple)
    with patch("alpharoute.brokers.paper.Paper") as order_place:
...
def test_multi_order_execute(users_simple, simple_order):
    order = simple_order
    multi = MultiUser(users=users_simple)
    with patch("alpharoute.brokers.paper.Paper.order_place") as order_place:
        for user in multi.users:
            # Patching  the order place method
            user.broker.order_place = order_place
...
def test_multi_order_execute_already_created(users_simple, simple_order):
    ...
    with patch("alpharoute.multi.MultiOrder.create") as create:
        order = simple_order
...
def test_multi_order_modify(users_simple, simple_order):
    ...
    multi = MultiUser(users=users_simple)
    order.execute(multi)
    with patch("alpharoute.brokers.paper.Paper.order_modify") as order_modify:
...
def test_multi_order_cancel(users_simple, simple_order):
    ...
    multi = MultiUser(users=users_simple)
    order.execute(multi)
    with patch("alpharoute.brokers.paper.Paper.order_cancel") as order_cancel:
        order.cancel()
